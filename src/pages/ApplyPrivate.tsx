...
const ApplyPrivate = () => {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      console.log('Private apply webhook response', res.status, res.ok, bodyText);

      if (!res.ok) {
        throw new Error(bodyText || `Webhook failed with status ${res.status}`);
      }

      toast.success("Request submitted successfully! We'll be in touch soon.");
      form.reset();
    } catch (err) {
      console.error('Private apply webhook submission failed', err);
        ...
        <div className="container mx-auto max-w-2xl px-4 sm:px-6 md:px-8">
          ...
          <div className="text-center mb-8 sm:mb-10 md:mb-12">
            ...
            <p className="font-body text-base sm:text-lg text-portal-navy/70 max-w-2xl mx-auto px-4">
              Tell us about your ideal Tuscan experience and we'll craft something extraordinary for you.
            </p>
          </div>
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea
                          {...field}
                          rows={6}
                          placeholder="Dates, number of guests, interests, any special requests..."
